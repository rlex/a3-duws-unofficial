...
str(_markername) setMarkerSize [(_radius)/1.5,(_radius)/1.5]; 


// make HQ zone notification trigger
_trg5=createTrigger["EmptyDetector",_position];
_trg5 setTriggerArea[(_radius)/1.5,(_radius)/1.5,0,false];
...
_unit sidechat format["This is %1 %2-%3, we are approaching the designated target area, ETA 1 minute",_fobname,_random1,_random2];
sleep 30;
_unit sidechat format["This is %1 %2-%3, ETA 30 seconds",_fobname,_random1,_random2];
sleep 30;
_unit sidechat format["This is %1 %2-%3, we have the target in visual, CBUs release in 15 seconds.",_fobname,_random1,_random2];
sleep 15;
_unit sidechat format["CBUs released, splash in 10 seconds."];
sleep 10;
...
//_bomb= _ammotype createVehicle [(_position select 0)+(random _radius)-(random _radius), (_position select 1)+(random _radius)-(random _radius), (_position select 2)]; //spawn shells
_bomb= _ammotype createVehicle [(_position select 0)+(random _radius)/1.5-(random _radius)/1.5, (_position select 1)+(random _radius)/1.5-(random _radius)/1.5, (_position select 2)]; //spawn shells
_bomb= _ammotype createVehicle [(_position select 0)+(random _radius)/1.5-(random _radius)/1.5, (_position select 1)+(random _radius)/1.5-(random _radius)/1.5, (_position select 2)]; //spawn shells
sleep ((random 1)/50);
