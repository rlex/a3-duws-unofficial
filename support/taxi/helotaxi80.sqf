...
if (commandpointsblu1 < 12) exitWith {
...
if (0 == _foundPickupPos select 0 && 0 == _foundPickupPos select 1) exitWith {hint "No valid LZ nearby"; sleep 5; _art = [player,"helo_taxi80"] call BIS_fnc_addCommMenuItem;};
...
_helo = createVehicle ["B_Heli_Transport_03_unarmed_F", [_posplayer select 0, (_posplayer select 1)+2000, 200],[], 0, "FLY"];
...
_helo animateDoor ['Door_R_source', 1];
sleep 3;
_helo animateDoor ['Door_L_source', 1];
sleep 2;
_helo animateDoor ['Door_rear_source', 1];
};
...
taxiCanTakeOff = false;
_pilot sidechat format["This is %1 %2-%3, get back in, I can't transport you're not inside the chopper !",_fobname,_random1,_random2];
_helo addAction ["<t color='#00b7ff'>Give a LZ to the pilot</t>", "support\taxi\mapclickhelo.sqf", "", 0, true, true, "", "vehicle _this == _target"];
} else {
_inVehCheck = false;
};
...
_lzPos = _this select 1;
waitUntil {getpos _helo distance _lzPos > 10};
_helo animateDoor ['Door_R_source', 0];
sleep 3;
_helo animateDoor ['Door_L_source', 0];
sleep 2;
