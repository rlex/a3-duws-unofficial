...
_unit		 = _this select 0; //[_this, 0, objNull, [objNull]] call BIS_fnc_param;
_altitude 	 = _this select 1; //[_this, 1, 2500, [2500]] call BIS_fnc_param;
_chemLight	 = _this select 2; //[_this, 2, true, [true]] call BIS_fnc_param;
_autoOpen 	 = _this select 3; //[_this, 3, true, [true]] call BIS_fnc_param;
_saveLoadOut = _this select 4; //[_this, 4, true, [true]] call BIS_fnc_param;
...
if (_unit == player) then {
	[_unit,_autoOpen] spawn {
		...
		if (_autoOpen) then {
			waitUntil {(getPosATL _unit select 2) < 150 || animationState _unit == "para_pilot" && alive _unit};
		...
		while {(getPosATL _unit select 2) > 5} do {
...
[_unit] spawn {
	...
	waitUntil {isTouchingGround _unit || (getPosATL _unit select 2) < 1 || !alive _unit}; //getPosATL getPosVisual
