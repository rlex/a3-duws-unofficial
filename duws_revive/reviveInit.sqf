...
// Compile scripts
getLoadout2 = compile preprocessFileLineNumbers 'duws_revive\get_loadout.sqf';
setLoadout2 = compile preprocessFileLineNumbers 'duws_revive\set_loadout.sqf';
                                                
// Save loadout every 5 seconds
[] spawn {
    while{true} do {
        if(alive player) then {
            saved_loadout = [player] call getLoadout2;
        };
    sleep 5;  
    };
};
