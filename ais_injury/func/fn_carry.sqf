...
_timenow = time;
waitUntil {sleep 1;!alive _injuredperson || {!alive _dragger} || {_dragger getVariable "tcb_ais_agony"} || {time > _timenow + 16}};
if (!alive _injuredperson || {!alive _dragger} || {(_dragger getVariable "tcb_ais_agony")}) then {
	...
	//_injuredperson attachTo [_dragger,  [-0,-0.1,-1.2], "RightShoulder"];
};

_dragger = _injuredperson getVariable "dragger";
waitUntil {sleep 1;!alive _injuredperson || {!alive _dragger} || {_dragger getVariable "tcb_ais_agony"} || {isNull _dragger}};
