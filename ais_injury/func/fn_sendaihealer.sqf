...
_delay = if (_callout) then {0 + (random 1)} else {5 + (random 10)};
sleep _delay;
...
While {alive _playerdown && {_playerdown getVariable "tcb_ais_agony"} && {_closestsquadmate distance _playerdown > 5} && {alive _closestsquadmate} && {!(_closestsquadmate getVariable "tcb_ais_agony")}} do {
	_delay = time + 1;
	...
	waitUntil {(_closestsquadmate distance _playerdown <= 5) || {time > _delay}};
};
