...
if (tcb_ais_show_3d_icons == 1) then {
	_icons = addMissionEventHandler ["Draw3D", {
		...
		} forEach units group player; //playableUnits;
	}];
};
...
_timeend = time + 2;
waitUntil {sleep 0.5;!isNil {_unit getVariable "BIS_fnc_feedback_hitArrayHandler"} || {time > _timeend}};	// work around to ensure this EH is the last one that was added
